...
@Api(basePath = "/api/aspectDmCustomer/v1", value = "Aspect Demand Service", description = "Operations on Aspect Demand Service", produces = "application/json", authorizations = { @Authorization(value = "dm_auth", scopes = { @AuthorizationScope(description = "Aspect Demand V1 authorization scope", scope = "dm") }) })
@SwaggerDefinition(basePath = "", externalDocs = @ExternalDocs(value = "Dig", url = "http://www.google.com"), host = "", info = @Info(title = "Aspect Demand Service", version = "1.0", contact = @Contact(name = "Aspect Demand Service Support", email = "manul30@yahoo.in", url = "http://www.google.com"), description = "Operations on Aspect Demand Service"))
@RestController
@RequestMapping(value = "/api/aspectDmCustomer/v1", produces = MediaType.APPLICATION_JSON_VALUE)
public class SpringBootAspectRestServiceController {

    @Autowired
    @Qualifier("DmServiceImpl")
    private DmServiceInterface dmService;

    private static final Logger LOG = LoggerFactory.getLogger(SpringBootAspectRestServiceController.class);
